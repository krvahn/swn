...
TableTools.prototype = {
	...
	"fnGetTableDataForXLS": function ( oConfig )
	{
		/* In future this could be used to get data from a plain HTML source as well as DataTables */
		if ( this.s.dt )
		{
			return this._fnGetDataTablesDataForXLS( oConfig );
		}
	},
			...
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
					...
					else if ( typeof mTypeData == "string" )
					{
						...
						// 내용 추가
						if( sLoopData == "#rowspan" || sLoopData == "#colspan" ) {
							sLoopData = "";
						}
					}
					else
	...
	"_fnGetDataTablesDataForXLS": function ( oConfig )
	{
		var dt = this.s.dt;
		var newLine, colSpan, rowSpan, align;
		
		newLine = this._fnNewline(oConfig);
		_sLastData = "<table border=1>" + this._fnNewline(oConfig);
		/*
		 * Header
		 */
		if ( oConfig.bHeader )
		{
			_sLastData += "<thead>" + newLine;
			// COLSPAN 및 ROWSPAN 여부 확인
	    	jQuery.each(dt.aoHeader, function(i, oRowData) {
	    		_sLastData += "<tr>" + newLine;	    		
	    		jQuery.each(oRowData.nTr.childNodes, function(j, oCellData) {
	    			if( oCellData ) {
	    				align = oCellData.align;
	    				colSpan = oCellData.colSpan;
	    				rowSpan = oCellData.rowSpan;
	    				if( colSpan > 0 || rowSpan > 0 ) {
		    				//console.log(oCellData.attr("style"));	    				
		    				sLoopData = oCellData.innerHTML.replace(/\n/g," ").replace( /<.*?>/g, "" ).replace(/^\s+|\s+$/g,"");
		    				//console.log(sLoopData);
		    				_sLastData += "<th colspan='" + colSpan +"' rowspan='" + rowSpan + "'";
		    				if( align != null && align != "" ) {
		    					_sLastData += " align='" + align + "'";
		    				}		    				
		    				_sLastData += ">";
		    				_sLastData += sLoopData;
		    				_sLastData += "</th>" + newLine;
		    			}
	    			}
	    		});
	    		_sLastData += "</tr>" + newLine;
		 ...
		 * Body
		 */
		_sLastData += "<tbody>" + newLine;
		// COLSPAN 및 ROWSPAN 여부 확인
    	jQuery.each(dt.aoData, function(i, oRowData) {
    		_sLastData += "<tr>" + newLine;
    		jQuery.each(oRowData.nTr.childNodes, function(j, oCellData) {
    			if( oCellData ) {
    				align = oCellData.align;
