...
public class PageLabMeasureValueWaterForm extends BaseActionImp {
	/**
	 * serialVersionUID
	 */
	private static final long serialVersionUID = 532518333828444636L;
	
	public PageLabMeasureValueWaterForm()
	{
	}
	...
	public String execute() throws Exception {
		...
		if( WebUtils.CheckNull(params.get("MODE"), "").equals("delete") ) {
			if( dao.deleteManualValueWaterDataList(params) > 0 ) {
			...
			if( itemCodeList.size() > 0 ) {
				if( dao.insertManualValueWaterDataList(params) > 0 ) {
